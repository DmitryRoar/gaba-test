...
Справочник пользователей на Next.js 16 + React 19. Feature-Sliced архитектура, RSC-first, Vitest + Playwright + Storybook, CI-гейт.
...
**Креды для входа** (бэкенд: публичный [dummyjson.com](https://dummyjson.com)):
...
Все переменные клиента имеют префикс `NEXT_PUBLIC_`. Серверные секреты валидируются через Zod в [src/shared/config/env.ts](src/shared/config/env.ts).
...
- **widgets**: композиции под страницу. Например, `UserProfile` с табами Carts / Posts / Todos.
- **app**: провайдеры, метаданные, layout-ы.

Импорты идут сверху вниз. Кросс-слайсовые импорты на одном слое запрещены. Правила декларируются в [eslint.config.mjs](eslint.config.mjs) через `eslint-plugin-boundaries` и проверяются на CI.
...
**Почему именно так.** Дефолт: RSC. Меньше JS на клиенте, серверные данные приходят прямо в компонент, нет водопадов. `'use client'` появляется только там, где нужна интерактивность (формы, тоглы, табы). ISR с лимитом на префетч это компромисс между холодным стартом и свежестью: за один билд не пререндерим 1000 страниц, но и не платим SSR за самые посещаемые.
...
**Почему не везде React Query.** RSC уже даёт серверный кэш и стриминг. Тянуть React Query в каждый список значит получить двойной кэш и лишний клиентский JS. Он остаётся для того, на что заточен: оптимистичный UX и кросс-компонентная инвалидация на клиенте.
...
- **Свои обёртки** в [src/shared/ui](src/shared/ui): Button, Input, DataTable, Modal, Toast, ThemeToggle, Skeleton и др. Все с `forwardRef`, merge через [`cn()`](src/shared/utils/cn.ts) на `tailwind-merge`.

**Почему HeroUI, а не shadcn/MUI.** shadcn это копипаста, которая со временем расходится с апстримом и тащит поддержку на нас. У MUI собственная тема и тяжёлый рантайм. HeroUI v3 имеет React Aria внутри и совместим с Tailwind v4 и нашими токенами. Стилизуем через классы, а не Theme Provider.
...
- **`@hookform/resolvers/zod`**: мост между ними.
...
**Почему пирамида именно такая.** Unit для логики (форматтеры, утилиты, schemas). Storybook для визуальных стейтов (loading / disabled / error) и a11y-чека. E2E только для пользовательских сценариев (login, search, navigation), без покрытия граничных случаев на каждом инпуте.
...
Конфиг в [.storybook/main.ts](.storybook/main.ts). Плагины: `@storybook/addon-docs`, `@storybook/addon-a11y`, `@storybook/addon-vitest`. Тогл темы прямо в тулбаре. Stories в [src/stories/ui](src/stories/ui).
...
[.github/workflows/ci.yml](.github/workflows/ci.yml) состоит из трёх job:

1. **unit**: `pnpm test:ci`, артефакт `coverage/`.
2. **e2e**: `pnpm build` + `pnpm test:e2e`. Кэш Playwright-бинарей по версии, кэш `.next/cache` по хешу исходников. При падении заливается артефакт `playwright-report/`.
